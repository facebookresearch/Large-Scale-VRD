...
if __name__ == '__main__':
    ...
    # for curr_iter in range(start_model_iter, 1):
        ...
        # training_stats.LogIterStats(curr_iter, lr)

    #     if (curr_iter + 1) % cfg.CHECKPOINT.CHECKPOINT_PERIOD == 0 and \
    #         curr_iter > start_model_iter:
    #         checkpoints[curr_iter] = os.path.join(
    #             output_dir, 'model_iter{}.pkl'.format(curr_iter)
    #         )
    #         nu.save_model_to_weights_file(checkpoints[curr_iter], model)
    #
    #     if curr_iter == start_model_iter + training_stats.LOG_PERIOD:
