...
def broadcast_parameters(model):
    ...
    on GPUs 1 through cfg.NUM_DEVICES - 1.
    """
    ...
    def _do_broadcast(all_blobs):
        assert len(all_blobs) % cfg.NUM_DEVICES == 0, \
            ('Unexpected value for NUM_DEVICES. Make sure you are not '
             'running single-GPU inference with NUM_DEVICES > 1.')
        blobs_per_gpu = int(len(all_blobs) / cfg.NUM_DEVICES)
